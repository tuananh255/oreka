...
const deleteImagesModel =async(req,res)=>{
  ...
  try {
    ...
    const imageIndex = room.images.findIndex(img => img.public_id === public_id);
    if (imageIndex === -1) {
    ...
    cloudinary.uploader.destroy(public_id, (error, result) => {
      ...
      res.json({ success: true, message: 'Image deleted successfully from Cloudinary and database', result });
    });
  } catch (error) {
    console.error(error);
    res.status(500).send({
...
module.exports = {
    uploadImages,
    deleteImages,deleteImagesModel
}